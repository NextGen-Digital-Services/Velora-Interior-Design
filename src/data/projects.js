...
const imagesByCategory = {
    'Luxury Villa': [img17, img49],
    'Penthouse': [img2, img26, img34, img42],
    'Corporate Office': [img27],
    'Luxury Retail': [img44],
    'Boutique Hotel': [img21],
    'Modern Kitchen': [img22, img46],
    'Executive Suite': [img15, img23, img31, img39, img47],
    'Wall Interior': [img8, img16, img24, img48]
};
...
export const projects = rawProjects.map((p) => {
    ...
    const categoryImages = imagesByCategory[p.category] || [img17];
    const heroImage = categoryImages[p.id % categoryImages.length];
    const afterImage = heroImage;
    const beforeImage = categoryImages[(p.id + 1) % categoryImages.length];
